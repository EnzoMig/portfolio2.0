...
canvas.width = Math.floor(window.innerWidth/15)*15;
canvas.height = Math.floor(window.innerHeight/15)*15;
// ctx.fillStyle = "#DBCFAB";
...
class Particle {
    constructor(effect){
        ...
        this.angle = 0;
        this.newAngle = 0;
        this.angleCorrector = Math.random() * 60 + 20;
...
class Effect {
    constructor(canvas, ctx) {
        ...
        window.addEventListener('resize', e => {
            console.log(e)
            this.resize(Math.floor(e.target.innerWidth/this.cellSize) * this.cellSize, Math.floor(e.target.innerHeight/this.cellSize) * this.cellSize);
        });
    }
    drawText(){
        ...
        this.context.fillText ('ENZO', this.width * 0.5, this.height * 0.5);
    }
    init(){
